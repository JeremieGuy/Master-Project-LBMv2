...
clotCoord = [17,23] # clot coordinates
...
def addResistingClotForce():
    ...
    # FF = zeros((9,nx,ny))
    # for i in range(9):
    #     FF[i,clotCoord[0]:clotCoord[1]+1,1:ny-1] = rho[clotCoord[0]:clotCoord[1]+1,1:ny-1]*(v[i,0]*(F[0] - K[0,clotCoord[0]:clotCoord[1]+1,1:ny-1]*u[0,clotCoord[0]:clotCoord[1]+1,1:ny-1]) + v[i,1]*F[1])
    #     FF[i,clotCoord[0]:clotCoord[1]+1,1:ny-1] = FF[i,clotCoord[0]:clotCoord[1]+1,1:ny-1] * (t[i] / cs2)

    # FF = zeros((9,nx,ny))
    # for i in range(9):
    #     FF[i,openPath] = rho[openPath]*(v[i,0]*(F[0] - K[0,openPath]*u[0,openPath]) + v[i,1]*F[1])
    #     FF[i,openPath] = FF[i,openPath] * (t[i] / cs2)
...
new_dir_monitoring = "./Monitoring/" + "[" + str(nx) + "," + str(ny) + "]" +  "_FF_viscosity=" + str(nulb) + "_Rho=" + str(initialDensity) 
