...
def plotResults():
    ...
    ax0.axhline(y = 0, color = 'r', linestyle = '--', linewidth=1) 
    ax0.axhline(y = 1+tubeSize+1, color = 'r', linestyle = '--', linewidth=1) 
    ...
    # velocities
    u_x = mean(u[0,1:nx-1,1:1+tubeSize+1].transpose(),axis=0)
    ax1.plot(x_full,u_x)
    ...
    # densities
    rho_x = mean(rho[1:nx-1,1:1+tubeSize+1],axis=1)
...
new_dir_monitoring = "./Monitoring/FF_loop_clotv2_FULLFF_viscosity=" + str(nulb) + "_Rho=" + str(initialDensity) 
