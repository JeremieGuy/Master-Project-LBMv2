...
nx, ny = 201, 21 # Number of lattice nodes.
...
def plotVelocityProfiles():
    ...
    #### Final velocities
    # Top  
    uTop = abs(u[0,50,1:ny-1])
    umaxTop = max(uTop)
    # Left cylinder
    # uMid = abs(u[1,249:300,75])
    # umaxMid = max(uMid)
    # Bottom
    uBot = abs(u[0,150,1:ny-1])
    umaxBot = max(uBot)
...
def drawClotVelocitiesDensityPressure():
    ...
    fig, (ax0, ax1, ax2, ax12) = plt.subplots(4, 1, figsize=(12, 15))
...
if savefiles : 
    new_dir_monitoring = "./Monitoring/test_old_tube_viscosity=" + str(nulb) + "_Re=" + str(Re) + "_it=" + str(maxIter)
