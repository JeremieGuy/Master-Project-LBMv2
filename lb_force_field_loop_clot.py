...
nx, ny = 200, 200 # Number of lattice nodes.
...
K_initial = [0,0.001] # initial resisting force of porous region K[2,x,y]
...
def plotResults():
    ...
    plt.clf()
    # x_full = arange((1+tubeSize),((1+tubeSize)+(ny-1-tubeSize-1)-(1+tubeSize)),1)
    x_full = arange(1+tubeSize,(ny-1-tubeSize),1)
    ...
    # 
    ax0.plot([nx-1-tubeSize,nx-1],[(ny//2-10),(ny//2-10)], linestyle='--', color='blue', linewidth=1)
    ax0.plot([nx-1-tubeSize,nx-1],[(ny//2+10),(ny//2+10)], linestyle='--', color='blue', linewidth=1)

    # interest zone
    ax0.plot([(nx-1-tubeSize), (nx-1-tubeSize)], [1+tubeSize,ny-1-tubeSize], color='red', linewidth=1) # top
    ax0.plot([(nx-1), (nx-1)], [1+tubeSize, ny-1-tubeSize], color='red', linewidth=1) # top
    ax0.plot([nx-1-tubeSize,nx-1],[1+tubeSize,1+tubeSize], color='red', linewidth=1)
    ax0.plot([nx-1-tubeSize,nx-1],[ny-1-tubeSize,ny-1-tubeSize], color='red', linewidth=1)

    fig.colorbar(im0, ax=ax0, orientation='vertical', label="Velocity [m/s]", pad=0.02)

    # velocities
    u_x = mean(u[0,(nx-1-tubeSize):nx-1,1+tubeSize:(ny-1-tubeSize)].transpose(),axis=1)
    ax1.plot(x_full,u_x)
    ax1.set_ylabel("Velocity u(x) mean [m/s]")
    ax1.axvline(x=(ny//2-10), color='blue', linestyle='--', linewidth=1)
    ax1.axvline(x=(ny//2+10), color='blue', linestyle='--', linewidth=1)

    # densities
    rho_x = mean(rho[(nx-1-tubeSize):nx-1,1+tubeSize:(ny-1-tubeSize)],axis=0)
    ax2.plot(x_full,rho_x)
    ax2.set_ylabel("Density rho(x) mean")
    ax2.axvline(x=(ny//2-10), color='blue', linestyle='--', linewidth=1)
    ax2.axvline(x=(ny//2+10), color='blue', linestyle='--', linewidth=1)
    ...
    ax3.axvline(x=(ny//2-10), color='blue', linestyle='--', linewidth=1)
    ax3.axvline(x=(ny//2+10), color='blue', linestyle='--', linewidth=1)
...
new_dir_monitoring = "./Monitoring/FF_loop_clot_viscosity=" + str(nulb) + "_Rho=" + str(initialDensity) 
