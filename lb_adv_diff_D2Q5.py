...
# Velocity directions vector for tPA, D2Q5
v_tPA = array([[ 1, 0], [ 0, 1], [ 0, 0], [ 0, -1], [ -1, 0]])

# Directionnal weighta for tPA, D2Q5
w_tPA = array([5/36, 5/36, 4/9, 5/36, 5/36])
...
mainDirectory, clotDirectory, velocityDirectory, clotForceDirectory, tPADensityDirectory = createRepositories(maxIter,nx,ny,viscosity,rho_initial,F_initial,K_initial)
...
fin, fout, _, u = getVariables("loop", nx, ny, viscosity, rho_initial, F_initial, K_initial, 65000)
...
for execTime in range(maxIter):
    ...
    for i in range(5):                                  # tPA
        tPAout[i, bounceback] = tPAin[4-i, bounceback]
    ...
    tPAin[2,:,:] = tPAout[2,:,:]                                    # i = 2
    tPAin[3,:,:] = roll(tPAout[3,:,:],-1,axis=1)                    # i = 3
    tPAin[4,:,:] = roll(tPAout[4,:,:],-1,axis=0)                    # i = 4
    ...
    if(execTime%1000==0):
        saveTPA(tPADensityDirectory, rhoTPA, execTime)
...
# saveVariables("loop", nx, ny, viscosity, rho_initial, F_initial, K_initial, maxIter, fin, fout, rho, u)