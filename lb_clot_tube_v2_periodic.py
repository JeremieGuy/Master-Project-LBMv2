...
maxIter = 89000 # Total number of time iterations.
...
def drawClotVelocitiesDensityPressure():
    ...
    fig, (ax0, ax1, ax2, ax12, ax3, ax4, ax6) = plt.subplots(7, 1, figsize=(12, 15))
    ...
    densityClot = mean(rho[120:180,1:ny-1],axis=1)
...
if savefiles : 
    new_dir_monitoring = "./Monitoring/tube_u_clot_v2_periodic_gamma=" +str(gamma) + "_it=" + str(maxIter)
...
for execTime in range(maxIter):
    ...
    for i in range(9):  
        ...
        fout[i,:] = (1 - gammaArray[:])*(fin[i,:] - omega * (fin[i,:] - feq[i,:])) + gammaArray[:]*fin[8-i,:]
        # bounceback
